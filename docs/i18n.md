...
This guide explains how to use internationalization in the bot using grammY's i18n plugin with Fluent format.
...
1. `/settings` → Language → Select preferred language
...
```text
# Good
wallet_success_msg = ✅ Wallet created successfully
...
# Wallet messages
wallet_success_msg = ✅ Wallet created successfully
wallet_fail_msg = ❌ Wallet creation failed
wallet_msg = 💰 Wallet: { $address }

# Settings messages
settings_msg = ⚙️ Settings
set_language_msg = 🌎 Select Language
```
...
1. **Change Language**: Use `/settings` → Language to test different languages
...
// Translation file
message = Hello, { $userName }!  // ✅ Correct
message = Hello, { $name }!      // ❌ Wrong variable name
```
...
wallet_msg = Wallet: `{ $address }`  # ✅ Correct
```
